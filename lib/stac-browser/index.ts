...
export class StacBrowser extends Construct {
  ...
  constructor(scope: Construct, id: string, props: StacBrowserProps) {
    ...
    const buildPath = this.buildApp(
      props,
      props.cloneDirectory || DEFAULT_CLONE_DIRECTORY
    );
    ...
    this.bucketDeployment = new s3_deployment.BucketDeployment(
      this,
      "BucketDeployment",
      {
  ...
  private buildApp(props: StacBrowserProps, cloneDirectory: string): string {
    ...
    try {
      console.log(
        `Checking if a valid cloned repo exists with version ${props.githubRepoTag}...`
      );
      execSync(`git checkout tags/${props.githubRepoTag}`, {
        cwd: cloneDirectory,
      });
    } catch (error) {
      ...
      if (fs.existsSync(cloneDirectory)) {
        throw new Error(
          `Directory ${cloneDirectory} already exists and is not a valid clone of ${githubRepoUrl}. Please delete this directory or specify a different cloneDirectory.`
        );
      }
    ...
    if (props.configFilePath) {
      ...
      if (!fs.existsSync(props.configFilePath)) {
        throw new Error(
          ...
          } does not exist. Current working directory is ${process.cwd()}`
        );
      }
      console.log(
        `Copying config file ${props.configFilePath} to ${cloneDirectory}/config.js`
      );
    ...
    console.log(
      `Building app with catalogUrl=${props.stacCatalogUrl} into ${cloneDirectory}`
    );
    execSync(`npm run build -- --catalogUrl=${props.stacCatalogUrl}`, {
      cwd: cloneDirectory,
    });
